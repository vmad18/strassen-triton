...
def benchmark_linear_layers(
    ...
    try:
        ...
        print(f"Sanity Check | Standard Linear Dtype: {linear_std.weight.dtype} | Strassen Linear Dtype: {linear_strassen.weight.dtype}")
...
def profile_linear_layers(dtype):
    ...
    configurations = [
        ...
        (1024, 1), (1024, 2),
        (2048, 1), (2048, 2),
        (4096, 1), (4096, 2), (4096, 3),
        (8192, 1), (8192, 2), (8192, 3), (8192, 4),
        (8192*2, 1), (8192*2, 2), (8192*2, 3), 
        (8192*2, 4), # <== if this takes too long, just delete it  
...
if __name__ == "__main__":
    if not torch.cuda.is_available():
        ...
        dtypes = ["bfloat16", "tf32", "fp32"]
        for dt in dtypes:
            if dt == "bfloat16":
                print("==> Using bf16")
                profile_linear_layers(torch.bfloat16)
            elif dt == "tf32":
                print("==> Using tf32")
                torch.backends.cuda.matmul.allow_tf32 = True 
                torch.backends.cudnn.allow_tf32 = True 
                torch.set_float32_matmul_precision("high")
                profile_linear_layers(torch.float32)
            else:
