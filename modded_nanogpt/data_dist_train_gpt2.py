...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.q_attn = LinearStrassen(self.n_embd, self.n_embd, bias=False)
        self.k_attn = LinearStrassen(self.n_embd, self.n_embd, bias=False)
        self.v_attn = LinearStrassen(self.n_embd, self.n_embd, bias=False)
        # output projection
        self.c_proj = LinearStrassen(self.n_embd, self.n_embd, bias=False)
...
class MLP(nn.Module):
    def __init__(self, config):
        ...
        self.c_fc = LinearStrassen(config.n_embd, config.n_embd, bias=False).cuda()
        self.c_fc.compile(mode = "max-autotune")
        
        # print(self.c_fc(torch.randn((1, config.n_embd, config.n_embd)).cuda()))

        self.c_proj = LinearStrassen(config.n_embd, config.n_embd, bias=False)
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets):
        ...
        x = checkpoint(self.transformer.wte, idx, use_reentrant = False)  # token embeddings of shape (b, t, n_embd)
        for block in self.transformer.h:
            x = checkpoint(block, x, block_mask, use_reentrant = False)
        x = checkpoint(norm, x, use_reentrant = False)

        # compute loss
        logits = softcap(checkpoint(self.lm_head, x, use_reentrant = False), cap=30)  # tanh logit softcap
...
def main(hparam_overrides=None, model_overrides=None, trial: optuna.Trial | None = None):
    ...
    class Hyperparameters:
        ...
        batch_size = 32 # 2 * 8  # global batch size, in sequences
        device_batch_size: int = 1  # batch size, in sequences, per device
        sequence_length: int = 8192 # 32_768  # sequence length, in tokens
        num_iterations: int = 3584  # number of iterations to run
        learning_rate: float = 0.0000877  # muon
        emb_learning_rate: float = 0.000498  # adam
        warmup_iters: int = 0
    ...
    print("allow_tf32 (cuda):", torch.backends.cuda.matmul.allow_tf32)
    ...
    ddp_model = FSDP(model, auto_wrap_policy=auto_wrap_policy, device_id=ddp_local_rank, use_orig_params=True)
    ...
    with maybe_profile(args.do_profile, rank=ddp_rank) as prof:
        for step in range(args.num_iterations + 1):
            ...
            if args.val_loss_every > 0 and ((step + 1) % args.val_loss_every == 0 or last_step):
                # stop the clock
                torch.cuda.synchronize()
            ...
            for i in range(1, train_accumulation_steps + 1):
                # forward pass
                with ctx:
                    loss = ddp_model(x, y) / train_accumulation_steps 
            ...
            if master_process:
                ...
                print0(
                    f'step:{step + 1}/{args.num_iterations} train_loss:{(train_loss.item() * train_accumulation_steps):.4f} train_time:{approx_time:.0f}ms step_avg:{approx_time / timed_steps:.2f}ms tokens_seen:{tokens_seen:.2e} tokens/sec:{tokens_per_second:.2e} {mem_str}',
                    console=True,
                )
                if not args.disable_wandb:
                    wandb.log(
                        {
                            'train_loss': train_loss.item() * train_accumulation_steps,
