...
def strassen_kernel_fp32_accum(
    ...
    for k in range(0, K, BLOCK_SIZE):
        ...
        M1 = tl.dot(A_11 + A_22, B_11 + B_22, allow_tf32=True)
        M2 = tl.dot(A_21 + A_22, B_11, allow_tf32=True)
        M3 = tl.dot(A_11, B_12 - B_22, allow_tf32=True)
        M4 = tl.dot(A_22, B_21 - B_11, allow_tf32=True)
        M5 = tl.dot(A_11 + A_12, B_22, allow_tf32=True)
        M6 = tl.dot(A_21 - A_11, B_11 + B_12, allow_tf32=True)
        M7 = tl.dot(A_12 - A_22, B_21 + B_22, allow_tf32=True)
...
def strassen_matmul_two_layers(A: torch.Tensor, B: torch.Tensor) -> torch.Tensor:
    ...
    # --- Level 1 M-Calculations (Applying Strassen Base for Level 2) ---
    # Each torch.mm in the original code is replaced by a call
    # to _strassen_base, which performs the second layer split and
    # uses torch.mm at the N/4 x N/4 level.
