...
def run_matmul_fp32_accum(A, B, C, BLOCK_SIZE=32, **kwargs):
    if len(A.shape) == 3:  # Batched
        batch_size, M, K = A.shape
        _, _, N = B.shape
        grid = (batch_size, triton.cdiv(M, BLOCK_SIZE),
        ...
        matmul_kernel_fp32_accum[grid](
            ...
            B.stride(0), B.stride(1), B.stride(2), BLOCK_SIZE, **kwargs)
        return
...
def run_winograd_strassen(A, B, C, BLOCK_SIZE=32, **kwargs):
    if len(A.shape) == 3:
        batch_size, M, K = A.shape
        _, _, N = B.shape
...
def run_old_winograd_strassen(A, B, C, BLOCK_SIZE=32, **kwargs):
    if len(A.shape) == 3:  # Batched
        batch_size, M, K = A.shape
        _, _, N = B.shape
        grid = (batch_size, triton.cdiv(M, BLOCK_SIZE), triton.cdiv(N, BLOCK_SIZE))
